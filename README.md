...
- **Persistencia**: todo se guarda en `localStorage`. Hay botones para exportar/importar un archivo `.json` con todo el estado, para sincronizar datos entre los dos celulares (por WhatsApp, por ejemplo). "Restablecer datos" vuelve a la plantilla original del viaje; "Vaciar todo" deja la app completamente en blanco (sin días, categorías ni gastos) para empezar de cero — ambos piden confirmación.
